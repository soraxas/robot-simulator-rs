...
fn main() {
    App::new()
        ...
        .add_systems(
            Update,
            visualize_components_for::<Transform>(bevy_ui_extras::Display::Side(
        ...
        .add_systems(
            ...
            |mut urdf_load_requests: ResMut<AssetSpawnRequestQueue<Urdf>>,
             ...
             asset_server: Res<AssetServer>| {
                ...
                let load_urdf_path = "robot_resources/panda/urdf/panda.urdf";
                // "/home/soraxas/research/stein_mpc/robot_resources/panda/urdf/p.urdf";

                cached_urdf.urdf = asset_server.load(load_urdf_path);

                urdf_load_requests.requests.push_front(AssetSpawnRequest {
                    source: load_urdf_path.to_owned().into(),
                    position: Transform::from_xyz(0.0, 5.0, 0.0),
                    ..Default::default()
                })
            },
        )
